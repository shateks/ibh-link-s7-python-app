...
parser_alarm_set_of_data = [
    (('m200.1 alarm 1'),     (('M', 200, None, 1, DataType.BOOL, None, None), 'alarm 1')),
    (('   m200.1   alarm 1'),     (('M', 200, None, 1, DataType.BOOL, None, None), 'alarm 1')),
    (('i123.1 alarm 2'),        (('I', 123, None, 1, DataType.BOOL, None, None), 'alarm 2')),
    (('  db300.dbx10.2 alarm 3'),         (('D', 300, 10, 2, DataType.BOOL, None, None), 'alarm 3'))
    ]

parser_alarm_set_of_data_should_fail = [
    'm200', 'MD30.2', 'i22', 'q3',
    'db20dbx12.2', 'd2.dbx3.2', 'db3,dbx40.2', 'db2.dbx3,2',
    'm23.8', 'db2.dbx23.8',
    'mw30real', 'db1.dbb3 dword', 'db1.dbb3 int','', 'qb123 range(0,300)', 'mW123 range(-2,300)',
...
class TestPaserVariablePlc(unittest.TestCase):
    ...
    @idata(parser_alarm_set_of_data)
    def test_parse_alarm(self, tuptup):
        _text = tuptup[0]
        self.assertEqual(tuptup[1], self.t_parser.parse_alarm(_text))

    @idata(parser_alarm_set_of_data_should_fail)
    def test_parse_alarm_fail(self, val):
        self.assertRaises(ValueError, self.t_parser.parse_alarm, val)
